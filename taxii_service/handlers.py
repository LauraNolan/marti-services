...
def execute_taxii_agent(hostname=None, https=None, feed=None, keyfile=None,
    ...
    response = client.callTaxiiService2(hostname, '/services/poll/', t.VID_TAXII_XML_11,
                                        poll_msg.to_xml())
    ...
    if response.getcode() != 200 or taxii_msg.message_type == tm11.MSG_STATUS_MESSAGE:
        ...
        if response.info().getheader('X-TAXII-Content-Type') == t.VID_TAXII_XML_10:
            ...
            response = client.callTaxiiService2(hostname, '/services/poll/', t.VID_TAXII_XML_10,
                                            poll_msg.to_xml())
            taxii_msg = t.get_message_from_http_response(response, poll_msg.message_id)
...
def to_stix(obj, items_to_convert=[], loaded=False, bin_fmt="raw", ref_id=None):
    ...
    for obj in items_to_convert:
        ...
        if obj_type == class_from_type('Event')._meta['crits_type']:
            ...
            for each in comm:
                camp[0].add_related_indicator(each)
...
def run_taxii_service(analyst, obj, rcpts, preview, relation_choices=[], confirmed=False, ref_id=None):
    ...
    for feed in destination_feeds:
        ...
        failed = True
        result = gen_send(tm11, client, stix_doc, hostname,
                          t.VID_TAXII_XML_11,
                          dcn = [feed[1]],
                          url = "/services/inbox/")
        if len(result) == 2:
